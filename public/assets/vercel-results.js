...
//
// Renders the Vercel mainland-China probe snapshot (results/vercel-latest.json)
// into the page. Shared by the main results viewer and the dedicated Vercel
// results page so the two never drift.
(function () {
  var PATHS = [
    ['frontend', 'Frontend — browser on the deployed site'],
    ['backend', 'Backend — server \u2192 Vercel API + Storage'],
  ...
  function renderEnv(data, dl) {
    ...
    if (data.demoSiteUrl) rows.push(['Demo site', data.demoSiteUrl]);
    if (storage.blobRegion) rows.push(['Blob region', storage.blobRegion]);
    if (storage.redisRegion) rows.push(['Redis region', storage.redisRegion]);
  ...
  function load() {
    var root = document.getElementById('vercel-root');
    if (!root) return;
    var envEl = document.getElementById('vercel-env');
    var section = document.getElementById('vercel');

    fetch('/results/vercel-latest.json', { cache: 'no-store' })
      .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function (data) {
        if (!(data.probes || []).length) {
          if (root.dataset.required === 'true') {
            root.appendChild(el('p', 'No Vercel snapshot available yet.'));
          }
          return;
        }
      ...
      .catch(function () {
        if (root.dataset.required === 'true') {
          root.appendChild(el('p', 'Could not load results/vercel-latest.json.'));
        }
      });
  }
