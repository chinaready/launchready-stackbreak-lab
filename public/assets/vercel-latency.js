...
//
// Renders the Vercel page-resource latency snapshot (results/vercel-resources-latest.json).
(function () {
  ...
  var KIND_LABELS = { endpoint: 'Endpoint', raw: 'Raw asset' };
  ...
  function renderEdge(data, dl) {
    ...
    var geo = edge.geo || {};
    var loc = [geo.city, geo.country && (geo.country.code || geo.country.name || geo.country)].filter(Boolean).join(', ');
    var rows = [
      ['Edge serverRegion', edge.serverRegion || 'unknown'],
      ['Edge geo', loc || 'unknown'],
      ['Blob region', storage.blobRegion || 'unknown'],
      ['Redis region', storage.redisRegion || 'unknown'],
      ['Measured from', (env.cloudProvider || 'unknown') + ' / ' + (env.cloudRegion || 'unknown')],
      ['Generated', fmtDate(data.generatedAt)]
    ];
    if (data.demoSiteUrl) rows.unshift(['Demo site', data.demoSiteUrl]);
  ...
  function load() {
    var root = document.getElementById('vercel-latency-root');
    if (!root) return;
    var edgeEl = document.getElementById('vercel-edge');
    var section = document.getElementById('vercel-latency');

    fetch('/results/vercel-resources-latest.json', { cache: 'no-store' })
      .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function (data) {
        if (!(data.resources || []).length) {
          if (root.dataset.required === 'true') {
            root.appendChild(el('p', 'No Vercel latency snapshot available yet.'));
          }
          return;
        }
      ...
      .catch(function () {
        if (root.dataset.required === 'true') {
          root.appendChild(el('p', 'Could not load results/vercel-resources-latest.json.'));
        }
      });
  }
