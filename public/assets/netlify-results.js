...
//   #netlify        optional wrapper, unhidden once a snapshot renders
//   #netlify-env    optional <dl> for environment provenance
//   #netlify-root   required container the path sections are appended to
(function () {
  var PATHS = [
    ['frontend', 'Frontend — browser on the deployed site'],
    ['backend', 'Backend — server \u2192 Netlify API'],
  ...
  function load() {
    var root = document.getElementById('netlify-root');
    if (!root) return;
    var envEl = document.getElementById('netlify-env');
    var section = document.getElementById('netlify');

    fetch('/results/netlify-latest.json', { cache: 'no-store' })
      .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function (data) {
        if (!(data.probes || []).length) {
          if (root.dataset.required === 'true') {
            root.appendChild(el('p', 'No Netlify snapshot available yet.'));
          }
          return;
        }
      ...
      .catch(function () {
        if (root.dataset.required === 'true') {
          root.appendChild(el('p', 'Could not load results/netlify-latest.json.'));
        }
      });
  }
